    ...
    if (column == "$and") {
      ...
      if (Array.isArray(value)) {
        ...
        if (Array.isArray(value?.between) && value?.between?.length == 2) {
          conditions.push(
            `${col} BETWEEN ${sanitize(value?.between?.[0])} AND ${sanitize(value?.between?.[1])}`,
          );
        }
        if (
          ...
          conditions.push(
            `${col} NOT BETWEEN ${sanitize(value?.notBetween?.[0])} AND ${sanitize(value?.notBetween?.[1])}`,
          );
        }
        if (Array.isArray(value?.inRange) && value?.inRange?.length == 2) {
          conditions.push(
            `${col} BETWEEN ${sanitize(value?.inRange?.[0])} AND ${sanitize(value?.inRange?.[1])}`,
          );
        }
