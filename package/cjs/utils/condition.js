        ...
        if (column == "$and") {
            ...
            if (Array.isArray(value)) {
                ...
                if (Array.isArray(value?.notIn) && value?.notIn?.length) {
                    conditions.push(`${col} NOT IN ${(0, sanitize_js_1.sanitize)(value?.notIn)}`);
                }
                if (Array.isArray(value?.in) && value?.in?.length) {
                    conditions.push(`${col} IN ${(0, sanitize_js_1.sanitize)(value?.in)}`);
                }
                if (Array.isArray(value?.between) && value?.between?.length == 2) {
                    conditions.push(`${col} BETWEEN ${(0, sanitize_js_1.sanitize)(value?.between?.[0])} AND ${(0, sanitize_js_1.sanitize)(value?.between?.[1])}`);
                }
                if (Array.isArray(value?.notBetween) &&
                    value?.notBetween?.length == 2) {
                    conditions.push(`${col} NOT BETWEEN ${(0, sanitize_js_1.sanitize)(value?.notBetween?.[0])} AND ${(0, sanitize_js_1.sanitize)(value?.notBetween?.[1])}`);
                }
                if (Array.isArray(value?.inRange) && value?.inRange?.length == 2) {
                    conditions.push(`${col} BETWEEN ${(0, sanitize_js_1.sanitize)(value?.inRange?.[0])} AND ${(0, sanitize_js_1.sanitize)(value?.inRange?.[1])}`);
                }
                ...
                if (value?.gte) {
                    conditions.push(`${col} >= ${(0, sanitize_js_1.sanitize)(value?.gte)}`);
                }
                if (value?.lte) {
                    conditions.push(`${col} <= ${(0, sanitize_js_1.sanitize)(value?.lte)}`);
                }
                if (value?.neq) {
                    conditions.push(`${col} != ${(0, sanitize_js_1.sanitize)(value?.neq)}`);
                }
