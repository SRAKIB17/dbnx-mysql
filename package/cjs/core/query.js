...
class Query extends execute_js_1.Execute {
  ...
  findAll(...args) {
    if (args.length === 0) {
      throw new Error(
        "No arguments provided to 'findAll'. Expected a table name or model.",
      );
    }
    let table = "";
    let Config = args[1] ?? {};
    ...
    if (typeof args[0] === "string") {
      ...
      throw new Error(
        "Invalid first argument: must be a table name or a Model class.",
      );
    }
  ...
  findOne(...args) {
    if (args.length === 0) {
      throw new Error(
        "No arguments provided to 'findOne'. Expected a table name or model.",
      );
    }
    let table = "";
    let Config = args[1] ?? {};
  ...
  update(...args) {
    if (args.length === 0) {
      throw new Error(
        "No arguments provided to 'update'. Expected a table name or model.",
      );
    }
    let table = "";
    let Props = args[1];
  ...
  delete(...args) {
    if (args.length === 0) {
      throw new Error(
        "No arguments provided to 'delete'. Expected a table name or model.",
      );
    }
    let table = "";
    let Props = args[1] ?? {};
