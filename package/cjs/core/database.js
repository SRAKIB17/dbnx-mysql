...
class DBnx {
    ...
    ref = Symbol("ref");
    constructor(...arg) {
        config_js_1.GlobalConfig.logger = typeof arg[1] === "function" ? arg[1] : arg[2];
    ...
    connect(props) {
        try {
            if (this.#usePool) {
                ...
                config_js_1.GlobalConfig.logger_function("✅ Connection has been established successfully.");
                return new handler_js_1.DBnxHandler({ pool: pool }, this.#dbConfig);
            }
            else {
                let connection = (0, mysql2_1.createConnection)(this.#dbConfig).promise();
                this.#connection = connection;
                props?.(undefined, "Connection has been established successfully.");
                config_js_1.GlobalConfig.logger_function("✅ Connection has been established successfully.");
                return new handler_js_1.DBnxHandler({ connection: connection }, this.#dbConfig);
            }
        }
        catch (error) {
