...
class DBnxHandler extends query_js_1.Query {
    ...
    async close() {
        if (this.db.pool) {
            ...
            config_js_1.GlobalConfig.logger_function("✅ Connection pool closed successfully.");
        }
        else if (this.db.connection) {
            config_js_1.GlobalConfig.logger_function("⏳ Closing database connection...");
            await this.db.connection.end();
            config_js_1.GlobalConfig.logger_function("✅ Database connection closed successfully.");
        }
        else {
            config_js_1.GlobalConfig.logger_function("⚠️ No active connection or pool to close.");
        }
    }
