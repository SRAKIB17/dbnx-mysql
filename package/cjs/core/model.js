...
class Model extends model_define_js_1.ModelDefine {
  ...
  static async create(...args) {
    ...
    const result = await this.dbInstance.execute(
      (0, query_1.insert)(this.dbTableIdentifier, values, options),
    );
    config_js_1.GlobalConfig.logger_function(
      `🆕 Create into: \`${this.dbTableIdentifier}\``,
    );
    return result;
  }
  static async findAll(Config) {
    ...
    config_js_1.GlobalConfig.logger_function(
      `📋 Find all from: \`${this.dbTableIdentifier}\``,
    );
    const result = await this.dbInstance.execute(
      (0, query_1.find)(this.dbTableIdentifier, Config),
    );
    return result;
  }
  static async findOne(Config) {
    ...
    config_js_1.GlobalConfig.logger_function(
      `🔎 Find one from: \`${this.dbTableIdentifier}\``,
    );
    let config = Config ?? {};
  ...
  static async update(Props) {
    ...
    config_js_1.GlobalConfig.logger_function(
      `✏️ Update from: \`${this.dbTableIdentifier}\``,
    );
    const result = await this.dbInstance.execute(
      (0, query_1.update)(this.dbTableIdentifier, Props),
    );
    return result;
  }
  static async delete(Props) {
    ...
    config_js_1.GlobalConfig.logger_function(
      `🗑️ Delete from: \`${this.dbTableIdentifier}\``,
    );
    const result = await this.dbInstance.execute(
      (0, query_1.destroy)(this.dbTableIdentifier, Props),
    );
    return result;
  }
...
WHERE
      ...
      ?.forEach((r) => {
        const defaultValue = r?.default
          ...
          : r?.null === "YES"
            ? " DEFAULT NULL"
            : "";
        constraintsQuery.add(
          `MODIFY COLUMN ${r?.field} ${r?.type}${defaultValue}`,
        );
      });
    if (constraintList?.length) {
      console.log(
    ...
    for (const constraint of constraintList) {
      ...
      switch (constraintType) {
        ...
        case "CHECK":
          console.warn(
            "\x1b[33m%s\x1b[0m",
            `CHECK constraints cannot be dropped directly. Consider alternative handling.`,
          );
          break;
        case "DEFAULT":
        case "NOT NULL":
          console.log(
            `Attempting to drop ${constraintType} on ${constraintName}`,
          );
          const column = columnMetadata.find(
            (col) => col.field === constraintName,
          );
          if (column) {
  ...
  static async drop() {
    config_js_1.GlobalConfig.logger_function(
      `🗑️ Dropping table: \`${this.dbTableIdentifier}\``,
    );
    ...
    config_js_1.GlobalConfig.logger_function(
      `✅ Table \`${this.dbTableIdentifier}\` dropped successfully.`,
    );
  ...
  static async sync(force = false) {
    ...
    config_js_1.GlobalConfig.logger_function(
      `🛠️ Synchronizing model: \`${this.dbTableIdentifier}\``,
    );
    await this.dropTableConstraints();
    if (force) {
      config_js_1.GlobalConfig.logger_function(
        "🚀 Force sync enabled. Dropping the table and re-creating it...",
      );
      await this.drop();
      config_js_1.GlobalConfig.logger_function(
        "✅ Model table re-created successfully.",
      );
    ...
    if (
      ...
      config_js_1.GlobalConfig.logger_function(
        "⚠️ No existing table or metadata found. Re-creating the table...",
      );
      await this.drop();
      config_js_1.GlobalConfig.logger_function(
        "✅ Model table created successfully.",
      );
      return this.errorHandle(await this.dbInstance.execute(this.ddlQuery));
    }
    ...
    for (const existingKey in columnData) {
      if (
        !(existingKey in this.modelAttributes) &&
