...
export function isTableInJoin(sql, tableName) {
  sql = sql
    .replace(/(--[^\n]*|\/\*[\s\S]*?\*\/)/g, "")
    .toLowerCase()
    .trim();
...
export function parseJoins(joins) {
  ...
  joins?.forEach((join) => {
    ...
    const tableEntries = Object?.entries(tables || {}).filter(
      ([key]) => key !== "type" && key !== "on" && key !== "operator",
    );
    if (type) {
      ...
      if (tableEntries.length !== 2) {
        throw new Error(
          `❌JOIN requires exactly two tables for a relation, but found ${tableEntries.length} or condition not found`,
        );
      }
      ...
      if (tableEntries.length !== 2) {
        throw new Error(
          `❌ JOIN shorthand requires exactly two tables, but found ${tableEntries.length} or condition not found`,
        );
      }
      const [[table1, column1], [table2, column2]] = tableEntries;
      let primary = isTableInJoin(relation, table1) ? table2 : table1;
