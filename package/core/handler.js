...
import { parseMySQLUrl } from "../utils";
import { dbnxCondition } from "../utils/condition";
export class DBnx {
    #pool = null;
    #connection = null;
    #query = "";
    ...
    connect(props) {
        try {
            if (this.#usePool) {
                this.#pool = createPool(this.#dbConfig).promise();
            }
            else {
                this.#connection = createConnection(this.#dbConfig).promise();
            }
            if (props) {
    ...
    create(...args) {
        ...
        this.#query += insert(table, values, options);
